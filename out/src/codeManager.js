...
class CodeManager {
    ...
    runExecutable(fileUri, extensionUri) {
        ...
        if (this._config.get("clearAssistedTerminal")) {
    ...
    getCodeFileAndExecute(fileExtension, executor, appendFile = true, runMode = false) {
        let selection;
    ...
    executeCommandInOutputChannel(executor, appendFile = true) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const showOutputTab = this._config.get("showOutputTab");
            const decodeOutput = this._config.get("decodeMap")[this._languageId] || this._config.get("defaultDecode");
            if (showOutputTab){this._outputChannel.show(this._config.get("preserveFocus"));}
    ...
    MinimizeOutCommand(command, executor){
        vscode.window.showInformationMessage(executor)
    }
}
